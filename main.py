...
def prepare_parser() -> None:
    ...
    stock_data = generate_stock_interest_data(
        config["data"]["time_steps"], config["data"]["seed"]
    )
    ...
    v = transform_to_uniform(
        inflation_data["Unemployment"], config["data"]["time_steps"]
    )
    logging.info("Fitting Student-t copula...")
    copula = fit_studentt_copula(u, v)
    u_future = np.random.uniform(
        size=config["copula"]["inflation_unemployment"]["n_samples"]
    )
